...
(function (undefined) {
	...
	com.ui.window = Extend(com.baseObject, {
		 ...
		 * 初始化函数
		 * @param option 属性
		 * @param option control DOM元素
		 * @param isDom 是否是dom元素, 如果是在html页面上布局, 则option自动解析
		 */
		 ...
		 */
		 ...
		 addStyle: function (tab, className, doCss) {
			 ...
			 if ( typeof tab.cssAry != "undefined" ) {
				 //检查重复
				 for ( var i = 0, len = tab.cssAry.length; l < len; i++ ) {
		 ...
		 delStyle: function (tab, className, doCss) {
			 ...
			 if ( vt === "string" ) {
				 ...
				 if ( this.isNoEmpty(value) && !isNaN(value) ) {
					 value = parseInit(value, 10);
				 } else {
					 value = defValue;
				 }
			 } else if ( vt != "number" ) {
		 ...
		 */
		 ...
		 body: function () {
			 if ( typeof this.winBody === "undefined" ) {
				 this.windBody = document.getElementsByTagName("body")[0];
			 }
			 return this.winBody;
		 },
		 /**
